...
public class ChangeMemberStatus {
		 ...
		 @Before("execution(public * com.spring.*.*Controller.Change_*(..))")
	     ...
	     @After("execution(public * com.spring.*.*Controller.Change_*(..))")
	     public void After(JoinPoint joinPoint) {
	    	 // 관리자가 본인의 권한을 바꿀때 권한을 받은 유저의 status 를 update해주고 본인의 권한을 일반사용자 or 부관리자로 변경시키는 after
	    	 
	    	 HttpServletRequest request = (HttpServletRequest)joinPoint.getArgs()[0]; //주업무 객체의 첫번째 파라미터인 HttpServletRequest를 가져온다.
			 HttpServletResponse response = (HttpServletResponse)joinPoint.getArgs()[1]; //dispatcher.forward를 위해 response 또한 파라미터로 지정해야 한다.
		 
			 HttpSession session = request.getSession();
			 MemberVO loginuser= (MemberVO)session.getAttribute("loginuser"); // admin 
			 
			 String userid = request.getParameter("userid"); // 관리자가 다른회원의 권한을 바꿀때 받아오는 userid
			 String team_idx = request.getParameter("team_idx");
			 String chguser = request.getParameter("chguser"); // 관리자가 본인의 권한을 바꿀때 받아오는 부관리자 userid
			 
			 System.out.println("after 확인용 userid 와 team_idx "+userid+"  "+team_idx);
			 System.out.println("after 확인용 userid 와 team_idx "+chguser+"  "+team_idx);
			 HashMap<String, String> map = new HashMap<String, String>(); 
			 map.put("userid", userid);
			 map.put("team_idx", team_idx);
			 map.put("login_userid", loginuser.getUserid());
			 
			 String oldAdminStatus = (String)session.getAttribute("oldAdminStatus");
			 System.out.println("확인용 after oldAdminStatus : "+oldAdminStatus);
			 
			 int n = 0;
				 
			 if("0".equals(oldAdminStatus)) { // 기존의 관리자는 일반사용자로 변경   
				      map.put("userid", loginuser.getUserid());
					  n = service.updateOldAdmin(map); 
					  session.removeAttribute("oldAdminStatus");
			 }
			 else if("1".equals(oldAdminStatus)){ // 기존의 관리자는 부관리자로 변경
				 	  map.put("userid", loginuser.getUserid());
					  n = service.chgTo2ndAdmin(map);
					  session.removeAttribute("oldAdminStatus");
			 }
		     ...
		     if(n > 0){
		    	 ...
		    	 int m = service.updateAdminUser(map); //관리자 권한을 받은 user를 ins_team_member테이블의 status 값변경 
